...
summary: 如果您打算使用NetApp Copy and Sync 將資料從物件儲存同步到物件儲存（例如，從 Amazon S3 同步到 Azure Blob），則需要在建立同步關係之前準備資料代理程式群組。 
---
= 準備資料代理程式以在NetApp Copy 和 Sync 中的物件儲存之間同步數據
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
如果您打算在NetApp Copy and Sync 中將資料從物件儲存同步到物件儲存（例如，從 Amazon S3 同步到 Azure Blob），則需要在建立同步關係之前準備資料代理程式群組。
